...
def run_headless_benchmark(batch_size: int = 100) -> None:
    ...
    print(f"  * Total Settled Clusters:      {len(all_clusters)} clusters (0 unhandled orphans)")
    print("  " + "-" * 72)
    print(f"  * Double-Entry Invariant Gate: [PASSED] (sum(Debits) - sum(Credits) == INR 0.00)")
    print(f"  * Statutory GST Invariant:     [PASSED] (round(MDR * 18%) == GST)")
    print(f"  * Cumulative Variance Delta:   INR {total_unresolved_variance_paise / 100.0:.2f} ({total_unresolved_variance_paise} paise float drift)")
    print(f"  * End-to-End Latency:          {latency_ms:.2f} ms")
    print(f"  * Cryptographic Merkle Root:   {merkle_root[:28]}... [SHA-256]")
    print(f"  * Audit Verdict:               [PASS] ALL TRANSACTIONS CRYPTOGRAPHICALLY BOUND")
    print("=" * 76)
